...
bad_words = pd.read_csv("https://raw.githubusercontent.com/pc3541/Subtitle-Cleaning/main/exclusion_words.csv", header=None)
...
def run():
    ...
    if input_srt is not None:
        ...
        for index, row in bad_words.iterrows():
            filtered_df_srt = pd.concat([filtered_df_srt, df_srt[df_srt['Subtitles'].str.contains(row.values[0])]]).drop_duplicates().reset_index(drop=True)
    ...
    if input_scc is not None:
        ...
        for index, row in bad_words.iterrows():
            filtered_df_scc = pd.concat([filtered_df_scc, df_scc[df_scc['Subtitles'].str.contains(row.values[0])]]).drop_duplicates().reset_index(drop=True)
