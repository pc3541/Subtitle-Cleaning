...
def run():
    ...
    df_srt = pd.DataFrame()
    df_scc = pd.DataFrame()
    filtered_df_srt = pd.DataFrame(columns=["Starts","Ends","Filtered subtitles"])
    filtered_df_scc = pd.DataFrame(columns=["Starts","Ends","Filtered subtitles"])
    ...
    if input_srt is not None:
        ...
        for word in bad_words:
            filtered_df_srt = pd.concat([filtered_df_srt, df_srt[df_srt['Subtitles'].str.contains("word")]])
    ...
    if input_scc is not None:
        ...
        for word in bad_words:
            filtered_df_scc = pd.concat([filtered_df_scc, df_scc[df_scc['Subtitles'].str.contains("word")]])
