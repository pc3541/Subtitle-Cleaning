...
def run():
    ...
    if input_srt is not None:
        ...
        for index, row in bad_words.iterrows():
            filtered_df_srt = pd.concat([filtered_df_srt, df_srt[df_srt['Subtitles'].str.contains(row.values[])]]).drop_duplicates().reset_index(drop=True)
    ...
    if input_scc is not None:
        ...
        for index, row in bad_words.iterrows():
            filtered_df_scc = pd.concat([filtered_df_scc, df_scc[df_scc['Subtitles'].str.contains(row.values[])]]).drop_duplicates().reset_index(drop=True)
